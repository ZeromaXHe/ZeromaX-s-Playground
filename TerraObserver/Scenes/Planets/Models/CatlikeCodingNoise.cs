...
using TO.FSharp.Commons.Structs.Planets;
...
public partial class CatlikeCodingNoise : Resource, ICatlikeCodingNoise
{
    ...
    public Vector4 SampleNoise(Vector3 position) =>
        GetPixelBilinear(NoiseSourceImage!,
            (position.X - position.Y * 0.5f - position.Z * 0.5f) * NoiseScale / Planet.StandardScale,
            (position.Y - position.Z) * HexMetrics.OuterToInner * NoiseScale / Planet.StandardScale);
    ...
    public Vector3 Perturb(Vector3 position)
    {
        ...
        var x = vecX * (sample.X * 2f - 1f) * CellPerturbStrength
            * Planet.StandardScale * position.Length() / HexMetrics.StandardRadius;
