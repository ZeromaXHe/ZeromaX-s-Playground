...
type TileFlag =
    ...
    member this.GetRoadInterpolator (length: int) (idx: int) =
        if this.HasRoadThroughEdge idx then
            Vector2(0.5f, 0.5f)
        else
            Vector2(
                (if this.HasRoadThroughEdge <| HexIndexUtil.previousIdx length idx then
                     0.5f
                 else
                     0.25f),
                (if this.HasRoadThroughEdge <| HexIndexUtil.nextIdx length idx then
                     0.5f
                 else
                     0.25f)
            )
