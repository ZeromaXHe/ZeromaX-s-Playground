...
type FaceRepo(store: EntityStore) =
    ...
    interface IFaceRepo with
        override this.ForEachByChunky chunky forEachFace =
            FrifloEcsUtil.forEachEntity
            <| store.Query<FaceComponent>().AllTags(if chunky then &tagChunk else &tagTile)
            <| forEachFace

        override this.Add chunky (vertex1: Vector3) (vertex2: Vector3) (vertex3: Vector3) =
        ...
        override this.GetOrderedFaces (pointComponent: PointComponent) (pointEntity: Entity) =
        ...
        override this.Truncate() =
            FrifloEcsUtil.truncate <| store.Query<FaceComponent>()
