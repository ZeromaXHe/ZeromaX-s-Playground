...
module LonLatGridCommand =
    ...
    let private drawLatitude
        ...
        draw90Degrees lonLatGrid surfaceTool color primeMeridianDirection west90Direction
        <| Vector3.Up * sin
        <| dash
        // 对向子午线
        let antiMeridianDirection = Vector3(-cos, 0f, 0f)

        draw90Degrees lonLatGrid surfaceTool color west90Direction antiMeridianDirection
        <| Vector3.Up * sin
        <| dash
        // 东经 90 度
        let east90Direction = Vector3(0f, 0f, -cos)

        draw90Degrees lonLatGrid surfaceTool color antiMeridianDirection east90Direction
        <| Vector3.Up * sin
        <| dash

        draw90Degrees lonLatGrid surfaceTool color east90Direction primeMeridianDirection
        <| Vector3.Up * sin
        <| dash
    ...
    /// <param name="color"></param>
    let private drawLongitude
        (lonLatGrid: ILonLatGrid)
        (surfaceTool: SurfaceTool)
        (longitudeRadian: float32)
        (color: Color)
        =
        let equatorDirection =
            Vector3(Mathf.Cos longitudeRadian, 0f, Mathf.Sin longitudeRadian)
        // 北面
        draw90Degrees lonLatGrid surfaceTool color equatorDirection Vector3.Up Vector3.Zero false
        // 南面
        draw90Degrees lonLatGrid surfaceTool color equatorDirection Vector3.Down Vector3.Zero false

    let doDraw (lonLatGrid: ILonLatGrid) : DoDraw =
        fun () ->
            ...
            for i in -180 + lonLatGrid.LongitudeInterval .. lonLatGrid.LongitudeInterval .. 180 do
                ...
                drawLongitude lonLatGrid surfaceTool longitudeRadian color
            ...
            for i in -90 + lonLatGrid.LatitudeInterval .. lonLatGrid.LatitudeInterval .. 90 do
                ...
                drawLatitude lonLatGrid surfaceTool latitudeRadian color false
            // 北极圈：北纬 66°34′
            if lonLatGrid.DrawArcticCircle then
                drawLatitude lonLatGrid surfaceTool
    ...
    let drawOnPlanet (planet: IPlanet) (lonLatGrid: ILonLatGrid) : DrawOnPlanet =
        fun () ->
            lonLatGrid.Radius <- planet.Radius + planet.MaxHeight
            doDraw lonLatGrid ()

    let toggleFixFullVisibility (lonLatGrid: ILonLatGrid) : ToggleFixFullVisibility =
        fun toggle -> lonLatGrid.FixFullVisibility <- toggle

    let onCameraMoved (lonLatGrid: ILonLatGrid) : OnCameraMoved =
        fun _ delta ->
